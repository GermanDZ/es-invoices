...
- [ ] (tester) Add the `compliance` test suite covering all seven requirements — validation rejection, alta XSD-conformance + per-group Desglose, huella reproduction, chain linkage + fork-safety (Postgres-gated race like T-012), signature verify/tamper, annulment reference — and run the full suite green. **(partial — 14 tests green for req 1/2/4/5-linkage/6/7 + Desglose structure; full suite 51 green. Remaining: req 3 XSD-conformance (needs vendored XSD + full RegFactu envelope) and the req-5 Postgres race.)**
