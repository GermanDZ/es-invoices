...
- [ ] Implement `signing.py` — XAdES-enveloped signature over a built record using `certificates.services.get_cert_material`, plus a verify helper; add `lxml` + the signing library to `requirements.txt`. **(deferred — external deps; see design.md §Handoff and handoff.md)**
...
- [ ] (tester) Add the `compliance` test suite covering all seven requirements — validation rejection, alta XSD-conformance + per-group Desglose, huella reproduction, chain linkage + fork-safety (Postgres-gated race like T-012), signature verify/tamper, annulment reference — and run the full suite green. **(partial — 11 tests green for req 1/2/4/5-linkage/7 + Desglose structure; XSD-conformance, signature verify/tamper, and the Postgres race remain with the signing box.)**
